...
function init_ac_assignments(m::PODNonlinearModel)
    ...
    for (cnt,i) in enumerate(keys(constr_pivot_assignment))
        if !isempty(constr_pivot_assignment[i])
            assignment[sort([j for j in constr_pivot_assignment[i]])] = cnt
        end
    end

    m.loglevel > 0 && println("Generated $(length(assignment)) Arc-Consistency Tasks.")
    return assignment
end
...
function init_optimal_arc_model(m::PODNonlinearModel)
    ...
    base_discretization = flatten_discretization(deepcopy(m.discretization))
    create_bounding_mip(m, use_disc=base_discretization, warmstart=false, cuts=false)
...
function arc_consistency(m::PODNonlinearModel, var_looked=[], partidx=[], var_left=[], examined=Set(), cuts=Dict())
    ...
    if !isempty(var_looked)
        ...
        if arc_model_status in [:Optimal, :UserObjLimit, :UserLimit, :Suboptimal]
            ...
            m.loglevel > 199 && println("VAR-PATH $(var_looked) || PART-PATH $(partidx) || $(arc_model_status) || $(arc_model_obj)")
        elseif arc_model_status in [:Infeasible, :InfeasibleOrUnbounded]
            @assert length(var_looked) == length(partidx)
            D = length(var_looked)
            one_cut = [(var_looked[i], partidx[i]) for i in 1:D]
            sort!(one_cut, by=x->x[1])
            println("Cut Generated $(one_cut)")
            cuts[one_cut] = true
            return
        else
            ...
            for i in 1:length(aff[:vars])
                ...
                if aff[:sense] == :(==) && var_coef > 0.0
                    ...
                    if eval_l_bound > m.l_var_tight[var_idx] + m.tol
                        ...
                        (m.loglevel > 99) && println("[VAR$(var_idx)] LB $(m.l_var_orig[var_idx]) => $(m.l_var_tight[var_idx])")
                    end
                    if eval_u_bound < m.u_var_tight[var_idx] - m.tol
                        exhausted = false
                        m.u_var_tight[var_idx] = eval_u_bound
                        (m.loglevel > 99) && println("[VAR$(var_idx)] UB $(m.u_var_tight[var_idx]) <= $(m.u_var_orig[var_idx])")
                    end
